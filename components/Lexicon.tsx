...
// Fix: Added React to imports to resolve 'Cannot find namespace React' error when using React.FC
const Lexicon: React.FC<LexiconProps> = ({ stories, progress }) => {
  ...
  // 使用系统发音，但固定语速为 0.9 以提高听力辨析度
  const speak = (text: string) => {
    window.speechSynthesis.cancel();
    const utterance = new SpeechSynthesisUtterance(text);
    utterance.lang = 'en-GB';
    utterance.rate = 0.9; // 稍微放慢，利于模仿
      ...
      <div className="mb-8 flex flex-col space-y-6">
        <div className="flex flex-col md:flex-row md:items-start justify-between gap-4">
          <div className="shrink-0">
            <h1 className="serif-font text-3xl md:text-4xl font-bold text-[#3D2B1F]">精英语料库</h1>
          </div>
        ...
        <div className="grid grid-cols-2 gap-4">
          <div className="bg-white rounded-2xl p-4 border border-[#F4ECE4] flex items-center space-x-3 shadow-sm">
            <div className="bg-amber-50 p-2 rounded-xl shrink-0"><Star className="w-5 h-5 text-amber-500 fill-amber-500" /></div>
            <div className="min-w-0">
              <p className="text-[10px] font-bold uppercase tracking-wider text-gray-400">已收藏</p>
              <p className="text-xl font-bold text-[#3D2B1F]">{progress.collectedWords.length}</p>
            </div>
          </div>
          <div className="bg-white rounded-2xl p-4 border border-[#F4ECE4] flex items-center space-x-3 shadow-sm">
            <div className="bg-green-50 p-2 rounded-xl shrink-0"><ShieldCheck className="w-5 h-5 text-green-500 fill-green-500" /></div>
            <div className="min-w-0">
              <p className="text-[10px] font-bold uppercase tracking-wider text-gray-400">已掌握</p>
              <p className="text-xl font-bold text-[#3D2B1F]">{progress.masteredWords.length}</p>
            </div>
          </div>
        </div>
      </div>
        ...
        <div className="p-4 border-b border-[#F4ECE4]">
          <div className="flex flex-col gap-4">
            <div className="flex bg-[#FAF7F2] p-1 rounded-xl w-full">
              <button onClick={() => setFilter('all')} className={`flex-1 px-2 py-2.5 text-[11px] font-bold rounded-lg transition-all ${filter === 'all' ? 'bg-white shadow-sm text-[#3D2B1F]' : 'text-gray-400'}`}>全部</button>
              <button onClick={() => setFilter('collected')} className={`flex-1 px-2 py-2.5 text-[11px] font-bold rounded-lg transition-all ${filter === 'collected' ? 'bg-white shadow-sm text-[#3D2B1F]' : 'text-gray-400'}`}>已收藏</button>
              <button onClick={() => setFilter('mastered')} className={`flex-1 px-2 py-2.5 text-[11px] font-bold rounded-lg transition-all ${filter === 'mastered' ? 'bg-white shadow-sm text-[#3D2B1F]' : 'text-gray-400'}`}>已掌握</button>
            </div>
            <div className="relative w-full">
              <Search className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-gray-300 pointer-events-none" />
              <input type="text" placeholder="搜索词汇..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} className="w-full pl-10 pr-4 py-3 text-xs bg-[#FAF7F2] rounded-xl border border-[#F4ECE4] outline-none text-[#3D2B1F]" />
            </div>
          </div>
        </div>
        ...
        <div className="divide-y divide-[#F4ECE4] max-h-[55vh] overflow-y-auto">
          {filteredWords.length > 0 ? (
              ...
              <div key={word.id} className="p-5 hover:bg-[#FAF7F2]/50 transition-colors flex items-center group">
                <div className="flex-grow min-w-0">
                  <div className="flex items-center space-x-2 mb-1">
                    ...
                    <button onClick={() => speak(word.word)} className="p-1 text-gray-300 hover:text-[#A67B5B] transition-colors"><Volume2 className="w-3.5 h-3.5" /></button>
                  </div>
            ...
            <div className="py-20 text-center">
              <div className="w-12 h-12 bg-[#FAF7F2] rounded-full flex items-center justify-center mx-auto mb-4"><Zap className="w-6 h-6 text-gray-200" /></div>
              <p className="text-gray-400 text-[11px]">暂无符合条件的语料</p>
            </div>
          )}
        </div>
      </div>
